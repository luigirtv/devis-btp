...
export default async function ApercuFiche({ searchParams }: PageProps<"/apercu/fiche">) {
  const { facture, email } = (await searchParams) as { facture?: string; email?: string };
  ...
  const lies = facture
    ...
    : [];
  return <FicheDocument document={d} client={CLIENTS_EXEMPLE[0]} parametres={PARAMETRES_EXEMPLE} emailConfigure={Boolean(email)} modifiable={!facture} lies={lies} />;
}
